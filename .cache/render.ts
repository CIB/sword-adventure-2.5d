...
function render(root: THREE.Object3D, file: string, yaw: number, pitch: number, W = 420, H = 520, scale = 150) {
  const rgb = new Uint8Array(W * H * 3).fill(200); const zb = new Float32Array(W * H).fill(-1e9);
  for (let i = 0; i < W * H; i++) { rgb[i * 3] = 120; rgb[i * 3 + 1] = 160; rgb[i * 3 + 2] = 120; }
  root.updateMatrixWorld(true);
  const cam = new THREE.Matrix4().makeRotationX(pitch).multiply(new THREE.Matrix4().makeRotationY(yaw));
  ...
  root.traverse((o) => {
    ...
    const pos = g.attributes.position; const idx = g.index;
    const n = idx ? idx.count : pos.count;
    const v = [new THREE.Vector3(), new THREE.Vector3(), new THREE.Vector3()];
    const nrm = new THREE.Vector3();
    for (let i = 0; i < n; i += 3) {
      for (let k = 0; k < 3; k++) { const vi = idx ? idx.getX(i + k) : i + k; v[k].fromBufferAttribute(pos, vi).applyMatrix4(m.matrixWorld).applyMatrix4(cam); }
      ...
      const sx = v.map((p) => W / 2 + p.x * scale), sy = v.map((p) => H - 40 - p.y * scale), sz = v.map((p) => p.z);
...
// pose it mid-charge, looking toward the camera's right
f.body.rotation.y = 0.6;
const n = f.stalk!.length; const c = 0.7;
f.stalk!.forEach((s, i) => { const w = (i + 1) / n; s.rotation.x = -0.16 * c * w * 0.9; s.rotation.z = 0.05 * w; });
f.head.rotation.x = 0.45 + 0.35 * c;
f.petals!.forEach((p) => p.rotation.x = -0.15 - 0.6 * c);
(f.mouth!.material as THREE.MeshToonMaterial).emissiveIntensity = c * 1.6;
const scene = new THREE.Group(); scene.add(f.root);
const knight = buildSoldier('sword'); knight.root.position.x = -1.3; scene.add(knight.root);
render(scene, '.cache/flower_front.png', 0.3, 0.45);
render(scene, '.cache/flower_side.png', 1.6, 0.3);
console.log('done');
