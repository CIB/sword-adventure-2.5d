...
export class Npc {
  ...
  private animate(moving: boolean) {
    ...
    else if (id === 'bard') { m.armL.rotation.set(-1.0, 0.2, -0.45); m.armR.rotation.set(-0.75 + Math.sin(this.t * 7) * 0.1, -0.45, 0.35); m.head.rotation.z = Math.sin(this.t * 2) * 0.1; m.body.position.y += Math.abs(Math.sin(this.t * 2)) * 0.015; }
